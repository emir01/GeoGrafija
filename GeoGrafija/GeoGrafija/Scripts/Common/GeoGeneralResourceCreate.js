...
var GeoGeneralResourceCreate = (function () {
    ...
    function init() {
        ...
        setTimeout(function () { clearMask(); }, 1000);
    }
    ...
    function setupUi() {
        ...
        $(".resourceLocationTypeWrapper").hide();
        ...
        var selectedResourceType = $("#ResourceTypeDropdown").closest("div").find(".dk_options").find("li.dk_option_current a").attr("data-dk-dropdown-value");

        //check for already selected location type to display 
        if (selectedLocaitonType && selectedLocaitonType != "") {
            $("#HiddenLocationTypeIDValue").val(selectedLocaitonType);

            $(".resourceLocationTypeWrapper").hide();

            $(".resourceLocationTypeWrapper").filter(function () {
                return $(this).attr("data-location-type-id") == selectedLocaitonType;
            }).slideDown();
        }

        //check for already selected resource type to display 
        if (selectedResourceType && selectedResourceType != "") {

            $(".resourceTypeWrapper").hide();
            $(".resourceTypeWrapper").filter(function () {
                return $(this).attr("data-id") == selectedResourceType;
            }).slideDown();
        }
    ...
    function onLocationTypeTypeChange(value, label) {
        ...
        $("#HiddenLocationTypeIDValue").val(value);
    }
    ...
    function saveResource(typeName, typeId, text, name) {
        ...
        locationTypeId = $("#HiddenLocationTypeIDValue").val();

        url = GeoAjax.GetUrlForAction(controller, action);
        dataArray = GeoAjax.JsonDataArray();

        dataArray.AddValue("LocationTypeId", locationTypeId);
    ...
    function insertDataInTable(msg) {
        ...
        var activeLocaitonTypeId = $("#HiddenLocationTypeIDValue").val();
        var activeLtContainer = $(".resourceLocationTypeWrapper").filter(function () {
            return $(this).attr("data-location-type-id") == activeLocaitonTypeId;
        });

        activeTable = $(".resourceTypeWrapper", activeLtContainer).filter(function () {
    ...
    function deleteRow(resourceId) {
        ...
        activeTable = $(".resourceTypeWrapper:visible", activeLtContainer).find(".resourcesDisplayTable");
